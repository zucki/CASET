...
public class Cocomo extends Calculation {
	...
	public  CocomoResults calculate() {
		...
		case Embedded:
			...
			result.setTimeToDev(2.5 * Math.pow(linesOfCode/1000, 0.32));
			break;
		case Organic:
			result.setPersonMonths(2.4 * Math.pow(linesOfCode/1000,1.05));
			result.setTimeToDev(2.5 * Math.pow(linesOfCode/1000, 0.38));
			break;
		case Semidetached:
			result.setPersonMonths(3.0 * Math.pow(linesOfCode/1000,1.12));
			result.setTimeToDev(2.5 * Math.pow(linesOfCode/1000, 0.35));
			break;	
		default:
